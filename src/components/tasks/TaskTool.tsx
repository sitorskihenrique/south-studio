...
import { filterTasks, getTodayTaskDay, getTomorrowTaskDay, taskSummary } from "@/lib/tasks/filters";
import { readTasks, writeTasks } from "@/lib/tasks/storage";
import { taskStatuses, type StudioTask, type TaskDay, type TaskDayFilter, type TaskDraft, type TaskStatusFilter } from "@/lib/tasks/types";
import { readCloudItems, replaceCloudItems } from "@/lib/supabase/data";
import { TaskCard } from "./TaskCard";
import { TaskFormSheet } from "./TaskFormSheet";
import { TaskSummary } from "./TaskSummary";

const dayTabs: TaskDayFilter[] = ["Hoje", "Amanhã", "Essa semana", "Concluídas"];
...
export function TaskTool() {
  ...
  function openNewTask() {
    ...
    const day =
      selectedDay === "Amanhã"
        ? getTomorrowTaskDay()
        : selectedDay === "Hoje" || selectedDay === "Essa semana" || selectedDay === "Concluídas"
          ? getTodayTaskDay()
          : selectedDay;
    setDraft({ ...emptyTaskDraft, day });
    setSheetOpen(true);
  }
      ...
      <div className="mx-auto max-w-[1320px] px-4 py-5 sm:px-8 lg:px-10 lg:py-9 fade-in">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_190px_auto]">
          <label className="relative">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar tarefa por nome" className="min-h-12 w-full rounded-2xl border border-zinc-200 bg-white/82 pl-10 pr-4 text-sm outline-none transition focus:border-zinc-400 focus:ring-4 focus:ring-zinc-950/5" />
          </label>
          <label className="relative">
            <ListFilter className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-400" size={17} />
            <span className="sr-only">Filtrar por status</span>
            <select value={status} onChange={(event) => setStatus(event.target.value as TaskStatusFilter)} className="min-h-12 w-full rounded-2xl border border-zinc-200 bg-white/82 pl-10 pr-4 text-sm font-medium text-zinc-700 outline-none transition focus:border-zinc-400">
              <option value="Todos">Todos os status</option>
              {taskStatuses.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
          </label>
          <button type="button" onClick={clearCompleted} disabled={!summary.completed} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-zinc-200 bg-white/82 px-4 text-sm font-semibold text-zinc-600 transition hover:border-zinc-300 hover:text-zinc-950 disabled:cursor-not-allowed disabled:opacity-40">
