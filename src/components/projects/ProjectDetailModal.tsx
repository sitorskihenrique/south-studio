...
const tabs: ProjectDetailTab[] = ["Visão geral", "Pré-produção", "Tarefas", "Orçamento", "Plano de Filmagem", "Arquivos/Referências"];
    ...
    <div className="fixed inset-0 z-[115] bg-zinc-100 sm:p-4 lg:p-6" role="dialog" aria-modal="true" aria-label={`Detalhe do projeto ${project.title}`}>
      <div className="mx-auto flex h-[100dvh] max-w-[1440px] flex-col overflow-hidden bg-white sm:h-full sm:rounded-[30px] sm:border sm:border-zinc-200">
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          {activeTab === "Visão geral" && <Overview project={project} />}
          {activeTab === "Pré-produção" && <PreProduction project={project} onChange={onChange} onSave={onSave} />}
          {activeTab === "Tarefas" && <ToolPanel title="Tarefas do projeto" description="Abra Tarefas para organizar as demandas deste projeto." href={`/tarefas?project=${encodeURIComponent(project.id)}`} icon={CheckSquare2} />}
          {activeTab === "Orçamento" && <ToolPanel title="Orçamento do projeto" description="Abra a Calculadora para preparar e salvar a proposta deste projeto." href={`/calculadora?project=${encodeURIComponent(project.id)}`} icon={WalletCards} />}
          {activeTab === "Plano de Filmagem" && <ToolPanel title="Plano de Filmagem" description="Organize diárias, sequências, takes e referências visuais." href={`/plano-de-filmagem?project=${encodeURIComponent(project.id)}`} icon={CalendarClock} />}
...
function Overview({ project }: { project: StudioProject }) {
  return (
